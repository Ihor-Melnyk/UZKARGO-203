...
function setClaimResponsible (extSysData) {
    ...
    if (extSysData.ClaimType_Title == 'Некоректне нарахування за перевезення вантажу та надані послуги') {
        ...
        var epdSearchResult;
        try {
            epdSearchResult = EdocsApi.getDictionaryData('EPD', null, [{attributeCode: 'epdNum', value: extSysData.ClaimDocs[0].DocNum}, {attributeCode: 'epdDate', value: moment(extSysData.ClaimDocs[0].DocDate).format("YYYY-MM-DD")}]);
        } catch (e) {}
...
function setRegProp(){
    ...
    if(edocsIncomeDocumentNumber && edocsIncomeDocumentDate){
        if(EdocsApi.getAttributeValue('ClaimNumber').value != edocsIncomeDocumentNumber)
            EdocsApi.setAttributeValue({code: 'ClaimNumber', value: edocsIncomeDocumentNumber, text: null});
        if(EdocsApi.getAttributeValue('ClaimDate').value != moment(edocsIncomeDocumentDate).format('DD.MM.YYYY'))
            EdocsApi.setAttributeValue({code: 'ClaimDate', value: moment(edocsIncomeDocumentDate).format('DD.MM.YYYY'), text: null});
    }
    else{
...
function onCardInitialize() {
    ...
    if(!CurrentDocument.inExtId){    
        ...
        if(EdocsApi.getCaseTaskDataByCode('Сheck')?.executionState == 'complete')
        ...
        if(!EdocsApi.getAttributeValue('DataFinish').value)
             EdocsApi.setAttributeValue({code: 'DataFinish', value: EdocsApi.getAttributeValue('edocsIncomeDocumentDate').value, text: null});
...
function onTaskExecuteСheck1(routeStage){
    if(routeStage.executionResult != 'rejected')
      setControlPropSendOutDoc (true);
}

function onTaskExecute(routeStage){
    if(EdocsApi.getCaseTaskDataByCode('Accepted'+EdocsApi.getAttributeValue('Rozriz').value)?.state == 'completed')
        setControlPropAccepted(true);
}
...
function onCardRendered(){   
    var DeliveryMethod = EdocsApi.getAttributeValue ('DeliveryMethod');
        if(DeliveryMethod && !DeliveryMethod.value)
            EdocsApi.setAttributeValue({code: 'DeliveryMethod', value: '2', text: null});
}
...
function setDocExecutionTerm(){
    ...
    if(EdocsApi.getAttributeValue('RailwayConnectionType').value == 'Міжнародне'){
        ...
        if(DocExecutionTerm .setHours(0,0,0,0) != val.setHours(0,0,0,0))
            EdocsApi.setAttributeValue({code: 'DocExecutionTerm', value: (val.toISOString()), text: null});
    }
    else{
...
function onBeforeCardSave() {
    ...
    if(!EdocsApi.getAttributeValue('DocExecutionTerm')?.value)
        EdocsApi.setAttributeValue({code: 'DocExecutionTerm', value: EdocsApi.getVacationPeriodEnd(new Date(), 10), text: null});
    ...
    if(DataVantazy){
        ...
        if(RailwayConnectionType == 'Міжнародне'){
            ...
            if(ClaimKind == 'Недотримання терміну доставки вантажу'){
                ...
                if(buf != new Date(EdocsApi.getAttributeValue('LastDate').value))
                    EdocsApi.setAttributeValue({code: 'LastDate', value: buf.toISOString(), text: null});
            }
...
function calculateTotalSum() {
    ...
    if(EdocsApi.getAttributeValue('edocsDocSum').value != contractAmountSum)
    EdocsApi.setAttributeValue({ code: 'edocsDocSum', value: (contractAmountSum > 0 ? contractAmountSum : 0 )});
}
/*
...
function onChangeInvoice_Claim_table(){
    ...
    if(table){
        var table2 = [], table3 = [], ClaimAmount2 = 0, ClaimAmount3 = 0;
        table.forEach((row) => {
            ...
            if(ReasonReturn == 'Прийнято'){
                ...
                table2.push([{code: 'Invoice_Claim_number2', value: row.find(x => x.code == 'Invoice_Claim_number').value, text: null},
                ...
                table3.push([{code: 'typeInvoice_Claim_table3', value: row.find(x => x.code == 'typeInvoice').text, text: null},{code: 'Invoice_Claim_number3', value: row.find(x => x.code == 'Invoice_Claim_number').value, text: null},{code: 'Invoice_Claim_Amaunt3', value: row.find(x => x.code == 'Invoice_Claim_Amaunt').value, text: null}]);                
        ...
        EdocsApi.setAttributeValue({code: 'edocsDocSum', value: (ClaimAmount3+ClaimAmount2)});
    }
    else{
...
function onTaskExecuteСheck(routeStage){
    if(routeStage.executionResult != 'rejected'){
        ...
        if(Invoice_Claim_table && Invoice_Claim_table.length > 0){
            Invoice_Claim_table.map(x=>(x.find(y=>y.code == "ReasonReturn").value)).every(s=> {if(s== null) {buf = true; str += '"Результат перевірки" таблиці, '}});
            Invoice_Claim_table.map(x=>(x.find(y=>y.code == "Invoice_Claim_number").value)).every(s=> {if(s== null) {buf = true; str += '"№  накладної" таблиці, '}});
            Invoice_Claim_table.map(x=>(x.find(y=>y.code == "Invoice_Claim_Amaunt").value)).every(s=> {if(s== null) {buf = true; str += '"Сума" таблиці, '}});
...
function onChangeResponsible(){  
    ...
    if(Responsible){  
        ...
        if(empl){
            if(EdocsApi.getAttributeValue('initiatorUnit').value != empl.unitName)
                EdocsApi.setAttributeValue({code: 'initiatorUnit', value: empl.unitName, text: null}); 
        }
    }    
    else{   
        if(EdocsApi.getAttributeValue('initiatorUnit').value)   
            EdocsApi.setAttributeValue({code: 'initiatorUnit', value: null, text: null});    
    }
}
